...
void CreateObject()
{
    // STRETCH GOAL APPLIED: Closed Bipyramid (sides = 4, top apex height = 0.7f, bottom apex at y = -2.0f)
    const int sides = 4;
    const float apexHeight = 0.7f;

    // 4 ring vertices + 2 apexes = 6 vertices * 3 components = 18 floats
    GLfloat vertices[18];
    ...
    // Bottom Apex (Index 5) - STRETCH GOAL ADDITION
    vertices[(sides + 1) * 3 + 0] = 0.0f;
    vertices[(sides + 1) * 3 + 1] = -2.0f;
    vertices[(sides + 1) * 3 + 2] = 0.0f;

    // 4 top side triangles (12 indices) + 4 bottom side triangles (12 indices) = 24 indices
    unsigned int indices[24];
    int indexCount = 0;
    int topApexIndex = sides;
    int bottomApexIndex = sides + 1;

    // TOP SIDES: for each i: i, topApex, (i + 1) % sides
    for (int i = 0; i < sides; ++i)
    {
        indices[indexCount++] = i;
        indices[indexCount++] = topApexIndex;
        indices[indexCount++] = (i + 1) % sides;
    }

    // BOTTOM SIDES: for each i: i, (i + 1) % sides, bottomApex (STRETCH GOAL APPLIED)
    for (int i = 0; i < sides; ++i)
    {
        indices[indexCount++] = i;
        indices[indexCount++] = (i + 1) % sides;
        indices[indexCount++] = bottomApexIndex;
    }
...
int main()
{
    ...
    while (!glfwWindowShouldClose(mainWindow))
    {
        ...
        // 24               the count of INDICES for 4-sided bipyramid (STRETCH GOAL APPLIED)
...
//          3. The draw index count (`18` or `24` for bipyramid) inside `glDrawElements()` tells OpenGL
//          how many indices to use when drawing the object.
//
//   b) How many vertices would you have written out WITHOUT indexing?
//      -> 18 vertices for standard 4-sided pyramid (6 triangles * 3 vertices), or 24 vertices for the bipyramid stretch goal (8 triangles * 3 vertices).
//
...
//
// =============================================================================
//  STRETCH GOAL (APPLIED IN CODE)
// =============================================================================
//    Turn your shape into a closed BIPYRAMID: added a second apex below the ring
...
//    Vertices: 6, Triangles: 8, Indices: 24.
// =============================================================================